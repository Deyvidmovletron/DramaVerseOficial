...
export function AdminEsqueciSenha() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setCarregando(true);
    try {
      await adminApi.post("/auth/admin/esqueci-senha", { email });
    } finally {
      setCarregando(false);
      setEnviado(true);
    }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-surface px-6 text-white">
      <div className="w-full max-w-sm rounded border border-white/10 bg-black/40 p-10 shadow-xl">
        <h1 className="mb-6 text-2xl font-bold text-brand">Esqueci minha senha</h1>
        ...
        {enviado ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-5">
              <label htmlFor="admin-esqueci-senha-email" className="sr-only">
                E-mail
              </label>
              <input
                id="admin-esqueci-senha-email"
                type="email"
                required
                placeholder="E-mail"
                value={email}
